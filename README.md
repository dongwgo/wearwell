...
## Qwen Lab — VLM이 무엇을 묻고 무엇을 답하는가 (개발용)

Seg Lab이 "옷을 어디까지 잡았나", Refine Lab이 "그 조각을 어떻게 그림으로 되돌리나"를
본다면, Qwen Lab은 파이프라인에서 **그림을 전혀 그리지 않는 한 칸**을 봅니다:
사진을 읽어 앱이 쓸 수 있는 스키마의 한국어 JSON으로 바꾸는 일.

```
사진 → 프롬프트(스키마 고정) → Qwen3-VL 생성(greedy) → JSON 파싱 → 옷장·추천·채점
```

화면은 두 덩이입니다. **보낸 것**(이미지 + 실제로 전송된 프롬프트 전문, 이번 요청에만
붙은 값은 노랗게 표시)과 **받은 것**(생성 원문 그대로 + 파싱된 필드 표). 넷 중 어느
일을 시킬지 고르면 그 태스크의 프롬프트와 토큰 예산이 그대로 화면에 뜹니다.

| 태스크 | 무엇을 맡았나 |
|---|---|
| 옷 한 벌 분석 | 카테고리·색·소재·핏·디테일 → 옷장 검색과 추천이 읽는 필드 |
| 룩북 분해 | 착장을 옷 단위로 쪼개고 bbox(0~1000)로 위치까지. **사진 위에 박스를 겹쳐 그립니다** |
| 체형 특징 | 전신사진에서 보이는 체형·비율·어깨선만 (키·몸무게 숫자는 금지) |
| 착장 결과 심판 | 합성 결과 채점. 정답 이미지가 없어 픽셀 지표로 못 재는 레이어 순서를 대신 판정 |

- **프롬프트는 백엔드에서 받아 옵니다**(`GET /api/vlm/prompts`). 탭이 프롬프트를 복사해
  두면 백엔드를 고칠 때마다 이 탭이 조용히 거짓말을 하게 됩니다
- **원문과 프롬프트는 `debug: true`일 때만** 응답에 실립니다. 앱 호출은 파싱된 JSON만
  쓰므로 그 무게를 지지 않습니다. 출력이 토큰 예산에서 잘렸는지(`truncated`)도 같이 옵니다
- Qwen은 NF4로도 8B라 **CPU에서는 사실상 돌지 않습니다.** Colab 백엔드를 쓰세요.
  첫 요청에 콜드 적재가 통째로 붙고, 응답의 `loadSeconds`로 구분됩니다
- `name`/`category`를 사진과 어긋나게 넣어 보면 "레이블은 참고만 하고 사진을 우선해"가
  실제로 지켜지는지 눈으로 확인할 수 있습니다
