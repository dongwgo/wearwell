...
독립 리뷰 두 건(백엔드/프론트엔드)에서 나온 항목을 모두 재현하고 고쳤습니다. 각 항목에는 회귀 테스트가 붙어 있고, 고친 코드를 되돌리면 그 테스트가 실패하는 것까지 확인했습니다.

**착장 품질에 직접 영향:**

- **속옷만 고르면 프롬프트가 "바지를 벗기라"고 말하고 있었습니다.** `_kept_slots_clause`가 카테고리로 슬롯을 봐서, 팬티가 '하의' 자리를 차지하고 정작 바지를 그대로 두라는 문장이 빠졌습니다. 결과 프롬프트는 "이 1벌만 입고 상의와 신발은 그대로" — 바지가 어디에도 없습니다. `body_slot()`을 쓰도록 고쳤습니다.
- **브라 + 셔츠 + 코트 → "the top then the top then the outerwear"** — 풀 수 없는 지시입니다. 속옷은 완전히 가려지는 게 목표라 보이는 레이어 스택에서 뺐습니다(base-layer 문장이 따로 맡습니다).
- **측면·후면 생성 실패가 정면까지 버렸습니다.** 두 번째·세 번째 생성은 참조가 많아 VRAM이 모자라기 가장 쉬운 지점인데, 거기서 터지면 이미 만든 정면 결과가 통째로 사라지고 500이 나갔습니다. 이제 부가 시점 실패는 로그만 남기고 만들어진 것을 돌려줍니다.

**리소스·가용성:**

- **4000px 사진 한 장에 180MB.** `has_room_below`가 전체 이미지를 float32로 복사한 뒤 아래 2%만 봤습니다. 자르고 나서 변환하도록 바꿔 5.9MB로 줄었습니다. 생성 크기보다 한참 큰 사진은 `downscale_to_budget()`으로 미리 줄입니다 — 패딩이 픽셀 수 검사 **이후에** 이미지를 다시 부풀리는 문제도 함께 없어집니다.
- **대기열이 스레드풀을 고갈시켰습니다.** 모든 엔드포인트가 동기 함수라 대기 요청이 슬롯(기본 40개)을 최대 300초 붙잡습니다. 40개가 쌓이면 `/api/health`조차 응답하지 못해 컨테이너 헬스체크가 추론 중인 서버를 죽입니다. `MAX_QUEUE_DEPTH`(기본 8)를 넘으면 기다리지 않고 503을 돌려줍니다.
- **401·429에 CORS 헤더가 없었습니다.** 인증 미들웨어가 CORS 바깥에서 응답을 반환해, 브라우저가 본문도 `Retry-After`도 읽지 못했습니다 — 프론트엔드는 "토큰 만료"와 "서버 다운"을 구분할 수 없었습니다. CORS를 가장 바깥으로 옮겼습니다.

**프론트엔드 상태:**

- **캐시 키가 아바타를 식별하지 못했습니다.** 사진 모드에서 `avatarMeasurements`는 성별로만 정해지는 고정 값이라, 사진을 바꿔도 키가 같아서 **앞사람의 착장 결과가 뒷사람 것으로** 표시됐습니다.
- **아바타화 결과가 사진을 바꿔도 남았습니다.** `photoAvatarImage`가 무효화되지 않아 "아바타로 보기"가 다른 사람의 몸을 보여줬습니다.
- **캐시 적중 시 `tryonContext`가 이전 착장을 가리켰습니다.** 같은 옷을 다시 고르면 회전·아바타 버튼이 옛 요청 번호를 들고 있다가, 눌러서 1분을 기다린 뒤 결과를 조용히 버리고 캡션이 "…1분쯤 걸려요"에 멈췄습니다.
- **아바타 생성에 stale 가드가 없었습니다.** 생성 중 성별을 바꾸면 버린 아바타가 되살아나 localStorage까지 덮어쓰고, 이후 모든 착장이 그 아바타로 만들어졌습니다.
- **VLM 체형 분석 `.then`이 나중 상태를 덮어썼습니다.** 사진 모드에서 시작한 분석이 치수 모드로 바꾼 뒤 도착해 `photoBased: true`를 붙였습니다.
- **비교 모드 위에서 드래그하면 룩북 원본 칸이 지워졌습니다.** 뷰어가 이전 결과를 들고 있었기 때문입니다. 비교·로딩 분기에서 뷰어 상태를 비웁니다.
- **프론트에서 잘라낸 아이템이 보고되지 않았습니다.** 백엔드는 자기가 받은 것만 셀 수 있는데, 개수 비교가 둘 다 자른 뒤 값이었습니다.
