...
def _resolve_device():
    ...
    if _device is None:
        ...
        if requested not in {"auto", "cuda", "cpu"}:
            raise ModelUnavailable("SEGMENTATION_DEVICE는 auto, cuda, cpu 중 하나여야 합니다.")
        ...
        if _device == "cuda" and not torch.cuda.is_available():
            raise ModelUnavailable("세그멘테이션에 CUDA를 요청했지만 GPU를 사용할 수 없습니다.")
    return _device
...
def status() -> dict:
    """모델을 새로 적재하지 않고 프로덕션 모델의 backend 준비 상태를 반환한다."""
    with _lock:
        return {
            "model": MODELS[PRODUCTION_MODEL].model_id,
            "modelKey": PRODUCTION_MODEL,
            "loaded": PRODUCTION_MODEL in _loaded,
            "loadedModels": list(_loaded),
            "device": _device,
        }
